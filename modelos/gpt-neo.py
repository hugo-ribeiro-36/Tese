...
with open('gpttext_125_full.txt', 'a') as f:
    for prompt in list:
        res = generator(promt_time, max_length=len(generator.tokenizer.encode(promt_time))+len(generator.tokenizer.encode("Russia has deployed hypersonic missiles — those that travel five times the speed of sound — in its war against Ukraine, while the U.S. is still developing hypersonic missiles.Russia’s hypersonic missiles are considered primitive. The U.S. versions are expected to have sophisticated abilities to glide, be launched by aircraft and maneuver to avoid defenses. Experts say they are a couple of years from being deployed.")), do_sample=True, temperature=0.9)
